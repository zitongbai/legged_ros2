...
#undef X
  ...
  void Simulate::RenderLoop()
  {
    ...
    while (!this->platform_ui->ShouldCloseWindow() && !this->exitrequest.load())
    {
      ...
      if (use_elastic_band_ == 1)
      {
        if (this->platform_ui->key_7_pressed_)
        {
          elastic_band_.length_ -= 0.1;
          this->platform_ui->key_7_pressed_ = false;
        }
        if (this->platform_ui->key_8_pressed_)
        {
          elastic_band_.length_ += 0.1;
          this->platform_ui->key_8_pressed_ = false;
        }
        if (this->platform_ui->key_9_pressed_)
        {
          elastic_band_.enable_ = !elastic_band_.enable_;
